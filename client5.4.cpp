...
{ //user may already exist
    ...
    else
    {
        ...
        {
            cout << "Unable to createa Acconut at the moment" << endl;
        }
    }
...
{ //user may already exist
    ...
    //deb(network_socketForLoginUser);
    // Send data to the socket
    //send(network_socket, &str, sizeof(str), 0);
    char buffer[MESSAGELEN];
    bzero(buffer, MESSAGELEN);
    ...
    // }
    deb(loginSucess);
    /*if (strcmp(buffer, "ok! send credentails") == 0)
    {
        ...
        if (recv(network_socketForLoginUser, server_reply, MESSAGELEN, 0) < 0)
        {
            msg("unable to get login Message");
        }
        else
...
void getInput()
{
    ...
    { // c. Login: login <user_id> <passwd>
        ...
        string passwd;
        cout << "Enter <user_id> <passwd>";
        cin >> user_id >> passwd;
        temp = imput + ' ' + to_string(user_id) + ' ' + passwd;
        deb(temp);
        arr = (char *)malloc(temp.length() * sizeof(char));
        strcpy(arr, temp.c_str());
        deb((char *)arr);
...
int main(int argc, char **argv)
{
    ...
    while (true)
        getInput();
    //this too has to be thread so that we can download from
