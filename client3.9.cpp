...
//failed on simultaneous write on single file
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
    ...
    while (1)
    {
        ...
        if ((fd_read != -1))
        {
            ...
            { //above may give issue
                ...
                write(newSocket, b, r);
                /*
                if (r <= 0 || offset_pos > amountUpToBeRead)
                    { //r <= 0 handles if end of file has occured without reading the entire block
                        cout << r << endl;
                        break;
                    }
            ...
            /*double file_size = sz;
            double FS_P = (sz * ((double)filePart - 1)) / (double)numberOfParts;
            unsigned long long start_loc = FS_P;
            if (start_loc % (512 * 1024) == 0)
            {

                unsigned long long amountToBeRead = ceil((sz / (((double)numberOfParts * 512 * 1024)))) * (512 * 1024);
                unsigned long long offset_pos = 0;
                offset_pos = start_loc;
                cout << "I am starting from:" << offset_pos << endl;
                //cout << "I have to read Upto:" << amountToBeRead << endl;
                unsigned long long total_read_so_far = 0;
                unsigned long long amountUpToBeRead = ceil((sz / (((double)numberOfParts * 512 * 1024))) * filePart) * (512 * 1024);
                cout << "I have to read Upto:" << amountUpToBeRead << endl;
                while (true)
                {
                    bzero(b, MESSAGELEN);
                    unsigned long long r = pread(fd_read, b, 512 * 1024, offset_pos);
                    ...
                    total_read_so_far += r;
                    //if (r <= 0 || total_read_so_far > amountToBeRead)
                    if (r <= 0 || offset_pos > amountUpToBeRead)
                    { //r <= 0 handles if end of file has occured without reading the entire block
                        cout << r << endl;
                        break;
                    ...
                    write(newSocket, b, r);
                    offset_pos += r;
                    cout << "read upto:" << offset_pos << endl;
                }
            }
            else if (start_loc % (512 * 1024) != 0)
            {
                unsigned long long amountUpToBeRead = ceil((sz / (((double)numberOfParts * 512 * 1024))) * filePart) * (512 * 1024);
                unsigned long long offset_pos = 0;
                offset_pos = start_loc;
