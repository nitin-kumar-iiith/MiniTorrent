...
unordered_map<string, string> mapFileName_Path;
...
void *stopShareNow(void *ptr)
{
    ...
    strcpy(buffer, str.c_str());
    deb(buffer);
    int network_socketForstopshare;
    // Create a stream socket
    network_socketForstopshare = socket(AF_INET, SOCK_STREAM, 0);
    // Initialise port number and address
    struct sockaddr_in server_address;
    server_address.sin_family = AF_INET;
    server_address.sin_addr.s_addr = INADDR_ANY;
    server_address.sin_port = htons(TRACKER1PORTNUM); //tracker's port number
    // Initiate a socket connection
    int connection_status = connect(network_socketForstopshare, (struct sockaddr *)&server_address, sizeof(server_address));
    // Check for connection error
    if (connection_status < 0)
    {
        msg("Error");
    ...
    // Send data to the socket
    int n = write(network_socketForstopshare, buffer, strlen(buffer));
    if (n < 0)
    {
    ...
    bzero(buffer, MESSAGELEN);
    if (recv(network_socketForstopshare, buffer, MESSAGELEN, 0) < 0)
    {
        msg("recv failed");
    }
    else
    ...
    if (strcmp(buffer, "Stopped Sharing") == 0)
    {
        msg("Sucess");
    }
    else
    {
        cout << "Unable to accept response at the moment" << endl;
    }
    // Close the connection
    close(network_socketForstopshare);
    pthread_exit(NULL);
    return NULL;
}
void *leaveGroup(void *ptr)
{ //user may not be part of Group or grp may not exist
    string str = (char *)ptr;
    char buffer[MESSAGELEN];
    bzero(buffer, MESSAGELEN);
    strcpy(buffer, str.c_str());
...
{ //user may already exist
    ...
    if (n < 0)
    {
        msg("ERROR writing logout command to Tracker");
        exit(0);
    }
    //Receive a reply from the server
    bzero(buffer, MESSAGELEN);
...
{ //user may already exist
    ...
    if (strcmp(buffer, "true") == 0)
    {
        ...
        if (infile == NULL)
        {
            msg("No download History of user");
        }
        else
        {
            mapGroupIDcompleted_Download.clear();
            msg("Loaded Download list of Current User");
            while (fread(&compl_Download, sizeof(struct downloadedFiles), 1, infile))
            {
                ...
                //mapGroupIDcompleted_Download[compl_Download.DownloadedFileName] = compl_Download.SourceGroupId;
                mapGroupIDcompleted_Download[compl_Download.SourceGroupId] = compl_Download.DownloadedFileName;
            }
            cout << "downloded files of user(" << user_id << ")"
                 << "are in :\n";
...
void *listFiles(void *ptr)
{
    ...
    if (buffer != "")
    {
        cout << "List of Files in given group ID are\n"
             << buffer << endl;
    }
    else
    {
...
{
    //group may not be in request list
    string str = (char *)ptr;
    deb(str);
    char buffer[MESSAGELEN];
    bzero(buffer, MESSAGELEN);
    ...
    else if (strcmp(buffer, "You Are Not Admin of Group") == 0)
    {
        cout << "You Are Not Admin of Group" << endl;
    }
    else
    {
        cout << "Error occured" << endl;
...
void *listRequests(void *ptr)
{
    ...
    if (n < 0)
    {
        msg("ERROR writing list_request command to Tracker");
        exit(0);
    }
    //Receive a reply from the Tracker
    bzero(buffer, MESSAGELEN);
    if (recv(network_socketForlistRequests, buffer, MESSAGELEN, 0) < 0)
    {
        msg("recv failed");
    ...
    if (buffer != "")
    {
        cout << "Pending Requests are:" << buffer << endl;
    }
    else
    {
        cout << "Unable to get group List or no group exists" << endl;
...
{ // k. Upload File: upload_file <file_path> <group_id>
    ...
    temp_s = "upload_file " + grpID + ' ' + usrID + ' ' + fileName + '\0';
    bzero(buffer, MESSAGELEN);
    ...
    if (n < 0)
    {
        msg("ERROR writing upload_file command to Tracker");
        pthread_exit(NULL);
        return NULL;
    }
    usleep(200);
    ...
    while ((read_byte = read(filedescriptor, fileContents, 524288)) > 0)
    {
        ...
        deb(SHA_1);
        sha1ofFilePieceWise.push_back(SHA_1);
        /*if (read_byte < 524288)
        {
            break;
        }*/
...
void *downloadFileHelper_Single(void *ptr)
{
    ...
    int fd_write;
    fd_write = open(destn_loc.c_str(), O_CREAT | O_WRONLY, 0777);
    if (fd_write == -1)
    {
...
void *downloadFileSingle(void *ptr)
{
    ...
    while (buffer[i] != ' ')
    {
        DestinPaths += buffer[i];
        i++;
    }
    deb(DestinPaths);
    if (DestinPaths[DestinPaths.length() - 1] != '/')
    {
        store_location = DestinPaths + '/' + fileName;
    }
    else
    {
        store_location = DestinPaths + fileName;
    ...
    // Send data to the socket
    bzero(buffer, MESSAGELEN);
    //tell Tracker which file to download and from which group
    string commands = "downfile " + fileName + ' ' + to_string(groupID) + ' ' + user_IID + '\0';
    strcpy(buffer, commands.c_str());
    int n = write(network_socketForDownloadFileInfo, buffer, strlen(buffer));
    if (n < 0)
    {
        msg("ERROR: writing to download Command to Tracker");
        pthread_exit(NULL);
        return NULL;
    }
    //Receive a reply from the server
    ...
    if (recv(network_socketForDownloadFileInfo, buffer, MESSAGELEN, 0) < 0)
    {
        msg("recv failed");
    }
    else
    ...
    if (strcmp(buffer, "no_User_has_complete_file") == 0)
    {
        msg("no active user in given group has entire file");
        pthread_exit(NULL);
        return NULL;
    }
    else
    {
        portNumberFRomWhereToDownload = buffer;
        string msgsd = "Download_sucess";
        n = write(network_socketForDownloadFileInfo, msgsd.c_str(), strlen(msgsd.c_str()));
        if (n < 0)
        {
            msg("ERROR: writing to download Sucess to Tracker");
            pthread_exit(NULL);
            return NULL;
        }
    ...
    word = "";
    int possibleportNum;
    set<string> portAddressOfPeopleHavingMyFile;
    for (auto x : portNumberFRomWhereToDownload)
    {
        if (x == ' ')
        {
            ...
            {
                portAddressOfPeopleHavingMyFile.insert(word);
            }
            word = "";
        }
        else
    ...
    vector<string> peoplewithmyfile;
    for (auto jtr = portAddressOfPeopleHavingMyFile.begin(); jtr != portAddressOfPeopleHavingMyFile.end(); jtr++)
    {
        string tempmps = (*jtr);
        peoplewithmyfile.push_back(tempmps);
    }
    unsigned int numberOfDestnPorts = peoplewithmyfile.size();
    //pthread_t threads[numberOfDestnPorts];
    pthread_t download_this;
    string temps;
    temps = to_string(1) + ' ' + fileName + ' ' + peoplewithmyfile[0] + ' ' + to_string(numberOfDestnPorts) + ' ' + store_location;
    char *ptrs = (char *)malloc(temps.length() * sizeof(char));
    //arr.push_back(ptrs);
    strcpy(ptrs, temps.c_str());
    //pthread_create(&threads[i], NULL, downloadFileHelper, &temps);
    pthread_create(&download_this, NULL, downloadFileHelper_Single, ptrs);
    usleep(200);
    string tempas = mapGroupIDcompleted_Download[grppp_IID];
    mapGroupIDcompleted_Download[grppp_IID] = tempas + ' ' + fileName;
    mapFileName_Path[fileName] = store_location;
    //sfs;
    //pthread_join(download_this, NULL);
    /*vector<char *> arr;
    char *ptrs;
    for (unsigned int i = 0; i < numberOfDestnPorts; i++)
    {
        temps = to_string(i + 1) + ' ' + fileName + ' ' + peoplewithmyfile[i] + ' ' + to_string(numberOfDestnPorts) + ' ' + store_location;
        ptrs = (char *)malloc(temps.length() * sizeof(char));
        arr.push_back(ptrs);
        strcpy(ptrs, temps.c_str());
        //pthread_create(&threads[i], NULL, downloadFileHelper, &temps);
    ...
    cout << "FileDownloaded" << endl;
    ////////////////////////////////
    /*
    FILE *infile;
    string storeLoc = to_string(user_id) + "_downloadLog.dat";
    struct downloadedFiles compl_Download;
    char DownloadedFileName[64];
    //struct groupJoinRequests groupJoinRequeststemp;
    infile = fopen(storeLoc.c_str(), "a");
    if (infile == NULL)
    {
    ...
    {
        compl_Download.DownloadedFileName,
            if (0 < fwrite(&compl_Download, sizeof(struct downloadedFiles), 1, infile))
        {
            ;
        }
        fclose(infile);
...
void getInput()
{
    ...
    { // c. Login: login <user_id> <passwd>
        ...
        {
            msg("You are already Logged In, logout First");
        }
    }
    ...
    { // h. Accept Group Joining Request: accept_request <group_id> <user_id>
        if (loginSucess == true)
        {
            ...
            temp = imput + ' ' + to_string(group_id) + ' ' + to_string(user_id_to_accept) + ' ' + to_string(user_id) + '\0';
            string temp2 = temp;
            deb(temp2);
    ...
    { // j. List All sharable Files In Group: list_files <group_id>
        if (loginSucess == true)
        {
            ...
            cout << "Enter <group_id>\n";
            cin >> group_id;
            temp = imput + ' ' + to_string(group_id) + '\0';
            deb(temp);
            arr = (char *)malloc(temp.length() * sizeof(char));
            if (arr == NULL)
            {
    ...
    { // k. Upload File: upload_file <file_path> <group_id>
        if (loginSucess == true)
        {
            ...
            temp = imput + ' ' + file_path + ' ' + to_string(group_id) + ' ' + to_string(user_id) + '\0';
            string temp2 = temp;
            arr = (char *)malloc(temp2.length() * sizeof(char));
            if (arr == NULL)
            {
    ...
    { // l. Download File: download_file <group_id> <file_name> <destination_path>
        if (loginSucess == true)
        {
            ...
            cout << "Enter <group_id> <file_name> <destination_path>" << endl;
            cin >> group_id >> fileName >> dest_path;
            temp = imput + ' ' + to_string(group_id) + ' ' + fileName + ' ' + dest_path + ' ' + to_string(user_id) + '\0';
            string temp2 = temp;
            arr = (char *)malloc(temp2.length() * sizeof(char));
            if (arr == NULL)
            {
            ...
            else
            {
                ...
                strcpy(arr, temp2.c_str());
                deb(arr);
                pthread_create(&handlingDownloadFile, NULL, downloadFileSingle, arr);
                usleep(200);
            }
            //fileName = "test.pdf";
            //cout << fileName << endl;
    ...
    { // o. Stop sharing: stop_share <group_id> <file_name>
        if (loginSucess == true)
        {
            ...
            int group_id;
            string file_name;
            cout << "Enter <group_id> <file_name>\n";
            cin >> group_id >> file_name;
            temp = imput + ' ' + file_name + ' ' + to_string(group_id) + ' ' + to_string(user_id) + '\0';
            arr = (char *)malloc(temp.length() * sizeof(char));
            if (arr == NULL)
            {
                cout << "Insufficient Memory, Unable to allocate\n";
            ...
            else
            {
                ...
                strcpy(arr, temp.c_str());
                pthread_create(&handlingInputTid, NULL, stopShareNow, arr);
                usleep(200);
            }
        }
        else
...
void *FileSendToPeerSingle(void *ptr)
{
    ...
    while (1)
    {
        ...
        if (filePart == 1)
        {
            ...
            string destn_loc;
            if (mapFileName_Path.find(fileName) != mapFileName_Path.end())
            {
                destn_loc = mapFileName_Path[fileName];
            }
            else
            {
                destn_loc = fileName;
            ...
            int fd_read;
            unsigned long long offset_pos = 0;
            fd_read = open(destn_loc.c_str(), O_RDONLY);
